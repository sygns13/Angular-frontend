...
@Component({
  selector: 'app-ventastopproductos',
  templateUrl: './ventastopproductos.component.html',
  styleUrls: ['./ventastopproductos.component.scss'],
  providers: [ConfirmationService, MessageService],
  encapsulation: ViewEncapsulation.None,
})
export class VentastopproductosComponent implements OnInit{
  ...
  constructor(private breadcrumbService: AppBreadcrumbService, private changeDetectorRef: ChangeDetectorRef , private ventaService: VentaService, 
    ...
    this.breadcrumbService.setItems([
      { label: 'Reportes' },
      { label: 'Reporte de Top de Productos Vendidos', routerLink: ['/reporte/top-productos-vendidos'] }
    ]);
  ...
  getTipoProductos() {
    ...
    this.tipoProductos.push({name: "Todos", code: 0});
    this.clsTipoProducto = {name: "Todos", code: 0};
  ...
  getMarcas() {
    ...
    this.marcas.push({name: "Todas", code: 0});
    this.clsMarca = {name: "Todas", code: 0};
  ...
  getPresentaciones() {
    ...
    this.presentaciones.push({name: "Todas", code: 0});
    this.clsPresentacion = {name: "Todas", code: 0};
  ...
  listarPageMain(p: number, s:number) {
    ...
    this.ventaService.getVentasTopProductosVendidos(this.filtroVenta, p ,s).subscribe(data => {
  ...
  evaluarFiltros(){
    ...
    if(this.clsTipoProducto != null && this.clsTipoProducto.code != "0") {
      this.filtroVenta.tipoProductoId = this.clsTipoProducto.code;
    }
    else{
      this.filtroVenta.tipoProductoId = null;
    }

    if(this.clsMarca != null && this.clsMarca.code != "0") {
      this.filtroVenta.marcaId = this.clsMarca.code;
    }
    else{
      this.filtroVenta.marcaId = null;
    }

    if(this.clsPresentacion != null && this.clsPresentacion.code != "0") {
      this.filtroVenta.presentacionId = this.clsPresentacion.code;
    }
    else{
      this.filtroVenta.presentacionId = null;
    }
  }
  ...
  printPDF(): void{
    this.exportsService.exportVentasTopProductosPDF(this.filtroVenta).subscribe(data => {
      ...
      a.href = fileURL;
      a.download = 'TopProductosVendidos.pdf';
      a.click();
  ...
  printXLS(): void{
    this.exportsService.exportVentasTopProductosXLSX(this.filtroVenta).subscribe(data => {
      ...
      document.body.appendChild(a);
      a.href = fileURL;
      a.download = 'TopProductosVendidos.xlsx';
      a.click();
      //window.open(fileURL);
    });
  }
